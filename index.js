...
// ================= RECONNECT & ANTI CONNECTION CLOSED ================= //
let reconnectAttempts = new Map();
let keepAliveInterval = null;
let lastPingResponse = new Map();
...
function removeActiveSession(botNumber) {
    try {
        if (fs.existsSync(SESSIONS_FILE)) {
            let activeSessions = JSON.parse(fs.readFileSync(SESSIONS_FILE));
        ...
        console.log(chalk.green(`✅ Session ${botNumber} dihapus`));
    } catch (error) {
        console.error("Error removing active session:", error);
    }
}

async function attemptReconnect(botNumber, chatId = null) {
    const attempts = reconnectAttempts.get(botNumber) || 0;
    
    if (attempts >= MAX_RECONNECT_ATTEMPTS) {
        console.log(chalk.red(`[RECONNECT] Gagal reconnect ${botNumber} setelah ${MAX_RECONNECT_ATTEMPTS} percobaan. Hapus session.`));
        ...
        if (chatId) {
            await bot.sendMessage(chatId, `❌ Sender ${botNumber} gagal terhubung setelah ${MAX_RECONNECT_ATTEMPTS} percobaan. Silakan /reqpair ulang.`);
        }
        return false;
    }
    
    reconnectAttempts.set(botNumber, attempts + 1);
    const delay = Math.min(RECONNECT_DELAY_BASE * Math.pow(2, attempts), 60000);
    
    console.log(chalk.yellow(`[RECONNECT] Mencoba reconnect ${botNumber} dalam ${delay/1000} detik (Percobaan ${attempts + 1}/${MAX_RECONNECT_ATTEMPTS})`));
    
    if (chatId) {
        try {
            await bot.sendMessage(chatId, `🔄 Mencoba reconnect sender ${botNumber}... (${attempts + 1}/${MAX_RECONNECT_ATTEMPTS})`);
        } catch(e) {}
    }
    ...
    setTimeout(async () => {
        try {
            ...
            console.error(chalk.red(`[RECONNECT] Error reconnect ${botNumber}:`, error.message));
            await attemptReconnect(botNumber, chatId);
        }
    }, delay);
...
async function ensureConnection(botNumber, chatId = null) {
    const currentSock = sessions.get(botNumber);
    
    if (!currentSock) {
        console.log(chalk.yellow(`[RECONNECT] Socket untuk ${botNumber} tidak ditemukan, mencoba reconnect...`));
        await attemptReconnect(botNumber, chatId);
        return false;
    }
    
    if (!currentSock.user || !currentSock.ws || currentSock.ws.readyState !== 1) {
        console.log(chalk.yellow(`[RECONNECT] Socket ${botNumber} dalam keadaan tidak sehat, mencoba reconnect...`));
        await attemptReconnect(botNumber, chatId);
        return false;
    }
    
    reconnectAttempts.set(botNumber, 0);
    connectionStatus.set(botNumber, true);
    return true;
}

async function sendKeepAlive(sock, botNumber) {
    ...
    try {
        ...
        await sock.query({
            ...
            content: [{ tag: 'presence', attrs: {} }]
        });
        
        lastPingResponse.set(botNumber, Date.now());
        connectionStatus.set(botNumber, true);
        return true;
    } catch (error) {
        console.log(chalk.yellow(`[KEEPALIVE] Gagal mengirim keep-alive ke ${botNumber}:`, error.message));
        connectionStatus.set(botNumber, false);
        return false;
    }
}
...
function startKeepAliveInterval() {
    ...
    keepAliveInterval = setInterval(async () => {
        for (const [botNumber, sock] of sessions.entries()) {
            const success = await sendKeepAlive(sock, botNumber);
            
            if (!success) {
                const lastResponse = lastPingResponse.get(botNumber) || 0;
                const timeSinceLastResponse = Date.now() - lastResponse;
                
                if (timeSinceLastResponse > 60000) {
                    console.log(chalk.yellow(`[KEEPALIVE] ${botNumber} tidak merespon selama ${timeSinceLastResponse/1000} detik, reconnect...`));
                    await attemptReconnect(botNumber, null);
                }
            }
        }
    }, KEEP_ALIVE_INTERVAL);
...
async function monitorConnections() {
    setInterval(async () => {
        for (const [botNumber, sock] of sessions.entries()) {
            const status = connectionStatus.get(botNumber);
            const isConnected = sock && sock.user && sock.ws && sock.ws.readyState === 1;
            
            if (!isConnected && status !== false) {
                console.log(chalk.yellow(`[MONITOR] Deteksi koneksi ${botNumber} bermasalah, reconnect...`));
                connectionStatus.set(botNumber, false);
                await attemptReconnect(botNumber, null);
            } else if (isConnected && status !== true) {
                connectionStatus.set(botNumber, true);
                reconnectAttempts.set(botNumber, 0);
            }
        }
...
async function cleanupInvalidSessions() {
    try {
        const activeNumbers = fs.existsSync(SESSIONS_FILE) ? JSON.parse(fs.readFileSync(SESSIONS_FILE)) : [];
        ...
        for (const botNumber of activeNumbers) {
            ...
            if (!fs.existsSync(credsPath)) {
                console.log(chalk.yellow(`[CLEANUP] Session ${botNumber} tidak valid, hapus dari daftar`));
                removeActiveSession(botNumber);
                continue;
            }
            ...
            if (fileAge > 30 * 24 * 60 * 60 * 1000) {
                console.log(chalk.yellow(`[CLEANUP] Session ${botNumber} sudah kadaluarsa (${Math.floor(fileAge / (24*60*60*1000))} hari), hapus`));
                removeActiveSession(botNumber);
                try {
                    fs.rmSync(sessionDir, { recursive: true, force: true });
                } catch (e) {}
...
`,
        ...
        console.log(chalk.yellow(`[CONNECTION] ${botNumber} terputus (${statusCode}), mencoba reconnect otomatis...`));
        connectionStatus.set(botNumber, false);
        sessions.delete(botNumber);
        setTimeout(async () => {
          await attemptReconnect(botNumber, chatId);
